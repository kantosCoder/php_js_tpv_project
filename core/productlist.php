...
if(isset($_SESSION["permission"])){
		if ($_SESSION["permission"]=="admin" || $_SESSION["permission"]=="user"){
			require ('dbjoin.php');
			$ask = "SELECT * from productos p, categoria c, categoria_producto k where c.id_categoria = k.id_categoria and p.id_producto = k.id_producto;";
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h2 class="page-title"><h2>&ensp;<i class="fas fa-boxes"></i> &ensp;Productos</h2>
                    </div>
                </div>
            </div>
		<div class="col-md-6">
			<marquee><h4>
			Aquí se muestra una lista de productos actualmente almacenados en el sistema.&ensp;&ensp;&ensp;             
			</h4>
			</marquee>
		</div>
                            ...
                            <div class="card-body">
								...
								echo('
                                    <tr>
										...
										<td>'.($row['IVA']).'%</td>
										<td>'.($row['stock']).'</td>
										<td><a href="productshort.php?cat='.($row['nombre_cat']).'">'.($row['nombre_cat']).'</a></td>
										...
										<script>function kk'.($row['id_producto']).'(){ if (confirm(\'¿Seguro que quieres eliminar el producto "'.($row['nombre']).'"\')) { window.location.href=\'engine/processing/productkill.php?mod='.($row['id_producto']).'\';} } </script>
										<button onclick="kk'.($row['id_producto']).'();return false" class="btn btn-outline-danger"><i class=""></i>&ensp;Eliminar producto</button></td>
										');}echo ('
                                    </tr> 
								');}
								echo('
